...
std::unique_ptr<chunk_base> construct_IHDR(std::span<const std::uint8_t> in, const std::vector<std::unique_ptr<chunk_base>>& chunks) {
	if (chunks.size()) { throw std::runtime_error{ "" }; }
	...
	if (position != in.data() + in.size()) { throw std::runtime_error{ "" }; }
	if (width == 0 || height == 0) { throw std::runtime_error{ "" }; }
	if (compression_method != 0) { throw std::runtime_error{ "" }; }
	if (filter_method != 0) { throw std::runtime_error{ "" }; }
	if (interlace_method > 1) { throw std::runtime_error{ "" }; }
	...
	if (!allowed_colour_type_bit_depth_pairs.contains({ colour_type, bit_depth })) { throw std::runtime_error{ "" }; }
...
std::unique_ptr<chunk_base> construct_PLTE(std::span<const std::uint8_t> in, const std::vector<std::unique_ptr<chunk_base>>& chunks) {
	...
	for (const std::unique_ptr<chunk_base>& i : chunks) {
		...
		if (i->get_type() == IDAT_chunk::type) { throw std::runtime_error{ "" }; }
	}
	if (!IHDR_exists) { throw std::runtime_error{ "" }; }
...
std::unique_ptr<chunk_base> construct_IEND(std::span<const std::uint8_t> in, const std::vector<std::unique_ptr<chunk_base>>& chunks) {
	if (in.size()) { throw std::runtime_error{ "" }; }
	bool IDAT_exists{ 0 };
	for (const std::unique_ptr<chunk_base>& i : chunks) {
		if (i->get_type() == IDAT_chunk::type) { IDAT_exists = 1; }
	}
	if (!IDAT_exists) { throw std::runtime_error{ "" }; }
