...
template <> std::uint_fast64_t to_integral_pixel(truecolour_8 in) {
	...
	return static_cast<decltype(integral_pixel_info{}.value)>(static_cast<std::uint_fast64_t>(in.red) << 16 | static_cast<std::uint_fast64_t>(in.green) << 8 | in.blue);
}
template <> std::uint_fast64_t to_integral_pixel(truecolour_16 in) {
	...
	return static_cast<decltype(integral_pixel_info{}.value)>(static_cast<std::uint_fast64_t>(in.red) << 32 | static_cast<std::uint_fast64_t>(in.green) << 16 | in.blue);
}
template <> std::uint_fast64_t to_integral_pixel(greyscale_with_alpha_8 in) {
	...
	return static_cast<decltype(integral_pixel_info{}.value)>(static_cast<std::uint_fast64_t>(in.grey) << 8 | in.alpha);
}
template <> std::uint_fast64_t to_integral_pixel(greyscale_with_alpha_16 in) {
	...
	return static_cast<decltype(integral_pixel_info{}.value)>(static_cast<std::uint_fast64_t>(in.grey) << 16 | in.alpha);
}
template <> std::uint_fast64_t to_integral_pixel(truecolour_with_alpha_8 in) {
	...
	assert(in.blue < (1 << 8) && "can\'t return integral_pixel_info when elements of in are greater than expected");
	assert(in.alpha < (1 << 8) && "can\'t return integral_pixel_info when elements of in are greater than expected");
	return static_cast<decltype(integral_pixel_info{}.value)>(static_cast<std::uint_fast64_t>(in.red) << 24 | static_cast<std::uint_fast64_t>(in.green) << 16 | static_cast<std::uint_fast64_t>(in.blue) << 8 | in.alpha);
}
template <> std::uint_fast64_t to_integral_pixel(truecolour_with_alpha_16 in) {
	...
	assert(in.blue < (1ull << 16) && "can\'t return integral_pixel_info when elements of in are greater than expected");
	assert(in.alpha < (1ull << 16) && "can\'t return integral_pixel_info when elements of in are greater than expected");
	return static_cast<decltype(integral_pixel_info{}.value)>(static_cast<std::uint_fast64_t>(in.red) << 48 | static_cast<std::uint_fast64_t>(in.green) << 32 | static_cast<std::uint_fast64_t>(in.blue) << 16 | in.alpha);
}
...
void write(std::uint8_t*& position, integral_pixel_info in) {
	...
	while (mask) {
		back -= 0b1000;
		*position++ = static_cast<std::uint8_t>((in.value & mask) >> back);
		mask >>= 0b1000;
	}
}