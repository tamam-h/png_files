...
void zlib_stream_handler::decompress() {
	...
	if (compressed_data.size() < 6) { throw std::runtime_error{ "" }; }
	std::uint_fast8_t first_byte{ compressed_data[0] };
	if ((first_byte & 0xF) != 8) { throw std::runtime_error{ "" }; }
	std::uint_fast8_t second_byte{ compressed_data[1] };
	if ((static_cast<std::uint_fast16_t>(first_byte << 8) | second_byte) % 31) { throw std::runtime_error{ "" }; }
	...
	if (adler32({ uncompressed_data.data(), uncompressed_data.data() + uncompressed_data.size() }) != checksum) { throw std::runtime_error{ "" }; }
}