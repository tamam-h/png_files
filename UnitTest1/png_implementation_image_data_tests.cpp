...
namespace png_implementation_image_data_tests {
	namespace image_data_tests {
		...
		public:
			TEST_METHOD(convert_to_test_1) {
				...
				Assert::IsTrue(time <= 0.0011, L"conversion should not cast each pixel if the end type is the same type as the start type");
			}
		...
		public:
			...
			TEST_METHOD(get_array_test_3) {
				...
				Assert::IsFalse(should_fail, L"can\'t convert internal representation with a reference to const image_data");
			}
		};
	}
}