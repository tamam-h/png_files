...
namespace png_implementation_zlib_stream_tests {
	...
	public:
		TEST_METHOD(adler32_test_1) {
			...
			Assert::IsTrue(static_cast<uint_fast32_t>(3271624791) == adler32({ buffer, buffer + sizeof(buffer) }));
		}
		TEST_METHOD(adler32_test_2) {
			...
			Assert::IsTrue(static_cast<uint_fast32_t>(1003032955) == adler32({ buffer, buffer + sizeof(buffer) }));
		}
	};
	namespace zlib_stream_handler_tests {
